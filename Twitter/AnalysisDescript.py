...
print ("Reading list of descriptions...")
rows = {}
for cat in headings:
    file = "Datasets\\" + cat + "descriptions.txt"
    with open(file) as f:
...
for cat in headings:
    ...
    for i in range(len(rows[cat])):
        ...
        for i in range(len(threegrams)):
            if threegrams[i][0] in stopwords:
                continue
...
for cat in headings:
    ...
    plt.title("High-frequency ngrams in " + cat1 + "'s descriptions") 
    plt.xlabel("Number of occurances")
    plt.ylabel("Ngrams")
    plt.savefig("Pictures\\descriptions\\"+cat1+"_wordcount.png")
    plt.close()
